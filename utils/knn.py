    ...
    def __init__(self, feats, k, index_path='', verbose=True, **kwargs):
        ...
        with Timer('[hnsw] query topk {}'.format(k), verbose):
            ...
            if os.path.exists(knn_ofn):
                ...
                self.knns = np.load(knn_ofn)['imagedata']
            else:
...
class knn_faiss(knn):
    def __init__(self,
                 ...
                 index_path='',
                 index_key='',
                 nprobe=128,  # find nprob number of most similar clusters
